...
import { BarcodeQRSection } from "./BarcodeQRSection"
...
export default async function ProductDetailPage(props: Props) {
  ...
  const [productRes, warehousesRes, inventoriesRes] = await Promise.all([
    // 1. ดึงข้อมูลสินค้าชิ้นที่ต้องการ พร้อมกับประวัติการเคลื่อนไหวของสต็อก (stock_movements) และซัพพลายเออร์
    supabase
      .from("products")
      .select(
        `*, stock_movements (id, created_at, type, quantity_change, notes, invoice_id, invoices ( invoice_number ), warehouses ( name )), suppliers ( id, name )`
      )
      .eq("id", id)
      .order("created_at", {
